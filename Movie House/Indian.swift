...
class Indian: UIViewController,GADBannerViewDelegate,GADInterstitialDelegate {
    ...
    override func viewDidLoad() {
        ...
        if connectedToNetwork() == true {
            Timer.scheduledTimer(timeInterval: TimeInterval(210), target: self, selector: #selector(Indian.loadad), userInfo: nil, repeats: false)
    ...
    func getdata(){
        Alamofire.request("https://bflix-f20b8.firebaseio.com/.json").responseJSON { (responseData) -> Void in
            if((responseData.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["indian"].arrayObject {
    ...
    @objc func setupPullToRefresh() {
        ...
        self.refreshControl.addTarget(self, action: #selector(Indian.refresh), for: UIControlEvents.valueChanged)
