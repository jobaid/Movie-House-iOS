...
//  Romantic Movie    Romantic Movie    Romantic Movie    Romantic Movie    Romantic Movie    Romantic Movie
...
class MusicViewController: UIViewController,GADInterstitialDelegate {
    ...
    override func viewDidLoad() {
        ...
        if connectedToNetwork() == true {
            Timer.scheduledTimer(timeInterval: TimeInterval(200), target: self, selector: #selector(MusicViewController.loadad), userInfo: nil, repeats: false)
    ...
    func getdata(){
        Alamofire.request("https://bflix-f20b8.firebaseio.com/.json").responseJSON { (responseData) -> Void in
            if((responseData.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["romantic"].arrayObject {
    ...
    @objc func setupPullToRefresh() {
        ...
        self.refreshControl.addTarget(self, action: #selector(MusicViewController.refresh), for: UIControlEvents.valueChanged)
